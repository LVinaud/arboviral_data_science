"""
Página: Próximos passos — roadmap do projeto e plano para artigo.
...
page_header(
    titulo=t("proximos.titulo"),
    descricao=t("proximos.descricao"),
    crumbs=t("proximos.crumbs"),
)
...
st.markdown(section_label(t("proximos.secao_resumo")), unsafe_allow_html=True)
c1, c2, c3 = st.columns(3)
with c1:
    st.markdown(nav_card(
        "C", t("proximos.card_curto_titulo"), t("proximos.card_curto_desc"),
    ), unsafe_allow_html=True)
with c2:
    st.markdown(nav_card(
        "M", t("proximos.card_medio_titulo"), t("proximos.card_medio_desc"),
    ), unsafe_allow_html=True)
with c3:
    st.markdown(nav_card(
        "L", t("proximos.card_longo_titulo"), t("proximos.card_longo_desc"),
    ), unsafe_allow_html=True)
...
tab_curto, tab_medio, tab_longo, tab_full = st.tabs([
    t("proximos.tab_curto"),
    t("proximos.tab_medio"),
    t("proximos.tab_longo"),
    t("proximos.tab_full"),
])
...
with tab_full:
    st.caption(t("proximos.tab_full_caption"))
    st.markdown(texto)
